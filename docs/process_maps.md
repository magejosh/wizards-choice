...
## StreamlinedMarketUI Workflow

```mermaid
flowchart TD
    Start[Open Market] --> LoadingPhase[Loading Phase]
    LoadingPhase --> Initial[Initial Loading Screen]
    Initial --> FetchMarkets[Fetch Market Data]
    FetchMarkets --> FilterMarkets[Filter Available Markets by Player Level]
    FilterMarkets --> DefaultSelection[Select Default Market]
    DefaultSelection --> ReadyUI[Market UI Ready]
    
    ReadyUI --> UIInteraction[User Interaction]
    
    UIInteraction --> Selection[Market Selection]
    UIInteraction --> Travel[Market Travel]
    UIInteraction --> TabNav[Tab Navigation]
    UIInteraction --> ModeToggle[Buy/Sell Mode Toggle]
    UIInteraction --> ItemSelect[Item Selection]
    UIInteraction --> Search[Search Items]
    UIInteraction --> Transaction[Transaction]
    UIInteraction --> Close[Close Market]
    
    Selection --> SelectMarket[Select Market from Dropdown]
    SelectMarket --> UpdateDescription[Update Market Description]
    
    Travel --> TravelLoad[Loading Travel]
    TravelLoad --> VisitMarket[Visit Selected Market]
    VisitMarket --> AttackCheck[Check for Market Attack]
    AttackCheck --> |Attack| ShowAttack[Show Attack Modal]
    AttackCheck --> |No Attack| LoadInventory[Load Market Inventory]
    
    ShowAttack --> AttackOptions[Fight or Flee Options]
    AttackOptions --> |Fight| StoreCombatant[Store Attacker Data]
    AttackOptions --> |Flee| FleeRoll[50% Chance Roll]
    
    StoreCombatant --> GotoBattle[Go to Battle]
    FleeRoll --> |Success| ReturnToStudy[Return to Wizard's Study]
    FleeRoll --> |Failure| StoreCombatant
    
    TabNav --> IngredientsTab[Ingredients Tab]
    TabNav --> PotionsTab[Potions Tab]
    TabNav --> EquipmentTab[Equipment Tab]
    TabNav --> ScrollsTab[Scrolls Tab]
    
    IngredientsTab --> UpdateInventoryDisplay[Update Inventory Display]
    PotionsTab --> UpdateInventoryDisplay
    EquipmentTab --> UpdateInventoryDisplay
    ScrollsTab --> UpdateInventoryDisplay
    
    ModeToggle --> BuyMode[Buy Mode]
    ModeToggle --> SellMode[Sell Mode]
    
    BuyMode --> ShowMarketItems[Show Market Items]
    SellMode --> ShowPlayerItems[Show Player Items]
    
    ItemSelect --> ShowDetails[Show Item Details]
    ShowDetails --> AdjustQuantity[Adjust Quantity]
    
    Search --> FilterItems[Filter Items by Name]
    FilterItems --> UpdateInventoryDisplay
    
    Transaction --> |Buy| BuyItem[Buy Item]
    Transaction --> |Sell| SellItem[Sell Item]
    
    BuyItem --> CheckGold[Check Player Gold]
    CheckGold --> |Sufficient| DeductGold[Deduct Gold]
    CheckGold --> |Insufficient| ShowError[Show Error Message]
    
    DeductGold --> AddToInventory[Add to Player Inventory]
    AddToInventory --> UpdateQuantity[Update Market Quantity]
    UpdateQuantity --> ShowSuccess[Show Success Message]
    
    SellItem --> AddGold[Add Gold to Player]
    AddGold --> RemoveFromInventory[Remove from Player Inventory]
    RemoveFromInventory --> ShowSuccess
    
    Close --> FinalAttackCheck[Final Attack Check]
    FinalAttackCheck --> |Attack| ShowAttack
    FinalAttackCheck --> |No Attack| ReturnToStudy
```