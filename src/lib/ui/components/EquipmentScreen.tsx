...
import { Equipment, EquipmentSlot, HandEquipment, Potion } from '../../types';
...
const EquipmentScreen: React.FC<EquipmentScreenProps> = ({
  ...
  const { gameState, updatePlayerEquipment, updatePlayerInventory, updatePlayerPotions, updatePlayerEquippedPotions } = useGameStateStore();
            ...
            {activeTab === 'potions' ? (
              ...
              <div className="potion-grid">
                {player.potions.length > 0 ? (
                  player.potions.map(potion => renderPotionCard(potion))
                ) : (
